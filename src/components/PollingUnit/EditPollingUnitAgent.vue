...
export default {
   ...
   methods : {
       async createAgent(){
           ...
           try {
               const createAgent = await axios.put(`https://v-guard.onrender.com/api/admin/unit/agent/${this.polling}/edit`, fd, {headers : {"Authorization": "Token "+localStorage.getItem('token')}})
               this.$swal({
                   icon:'success',
                   title: 'Success',
