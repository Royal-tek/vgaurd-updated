...
export default {
    ...
    methods: {
        ...
        async deleteAgent(){
            try {
                const deleteAgent = await axios.delete(`https://v-guard.onrender.com/api/admin/unit/agent/${this.agent}/delete`, {headers : {"Authorization": "Token "+localStorage.getItem('token')}})
                this.$swal({
                    icon:'success',
                    title: 'Success',
