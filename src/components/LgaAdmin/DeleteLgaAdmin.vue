        ...
        <div class="delete_box">
            ...
            <div class="delete_body">
                ...
                <div class="delete_action">
                    ...
                    <button @click="deleteLgaAdmin" class="btn_danger">Delete</button>
                </div>
            </div>
        </div>    
    </div>
...
export default {
    ...
    methods: {
        ...
        async deleteLgaAdmin(){
            try {
                const deleteLgaAdmin = await axios.delete(`https://v-guard.onrender.com/api/lgaAdmin/${this.lgaAdmin}/delete`, {headers : {"Authorization": "Token "+localStorage.getItem('token')}})
                this.$swal({
                    icon:'success',
                    title: 'Success',
                    text:'LGA ADMIN DELETED SUCCESSFULLY',
                    })
                this.closeModal()
                console.log(deleteLgaAdmin);
            } catch (error) {
                this.$swal({
                    icon:'error',
                    title: 'Failed',
